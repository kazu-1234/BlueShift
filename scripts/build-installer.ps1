# Build BlueShift installer (dotnet publish + Inno Setup)
# Usage:
...
dotnet publish (Join-Path $root "BlueShift.csproj") `
    -c Release `
    -p:Platform=x64 `
    -r win-x64 `
    --self-contained true `
...
$exe = Join-Path $publishDir "BlueShift.exe"
if (-not (Test-Path $exe)) {
    throw "BlueShift.exe missing in publish output: $publishDir"
}

Get-ChildItem $publishDir -Recurse -File | Where-Object {
    $_.Name -eq "onnxruntime.dll" -or
...
$setup = Join-Path $outDir ("BlueShift-v{0}-win-x64-setup.exe" -f $version)
...
$rootCopy = Join-Path $root ("BlueShift-v{0}-win-x64-setup.exe" -f $version)
